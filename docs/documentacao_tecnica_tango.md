...
Cada célula fica na classe `Cell`, que possui linha, coluna, valor e uma informação dizendo se ela era fixa desde o início do jogo. Isso é importante porque as células que já vêm preenchidas no arquivo de entrada não podem ser modificadas pelos algoritmos.
...
A classe `RuleValidator` junta essas regras e oferece dois tipos de validação: uma validação parcial, usada durante o backtracking, e uma validação completa, usada quando o tabuleiro já está preenchido.

## 2. Estratégia de Resolução por Força Bruta

Na força bruta, o algoritmo não tenta ser inteligente durante o preenchimento. Ele primeiro identifica todas as células vazias do tabuleiro e depois gera todas as combinações possíveis para essas células.
...
No código, isso fica na classe `BruteForceSolver`. O algoritmo usa uma máscara numérica para gerar as combinações. Para cada número da máscara, cada bit representa o valor de uma célula vazia. Quando o bit é 0, a célula recebe Sol. Quando o bit é 1, a célula recebe Lua.
...
Se a combinação for válida, o algoritmo termina e o tabuleiro fica resolvido. Se não for válida, ele passa para a próxima combinação.
...
Durante a recursão, depois de colocar um valor em uma célula, o algoritmo chama a validação parcial. Essa validação já elimina escolhas ruins antes de continuar. Por exemplo:
...
Nesse exemplo, a força bruta ainda é viável porque existem apenas 10 células vazias, gerando 1024 combinações possíveis.
...
Nesse caso, a força bruta não foi usada porque o tabuleiro 6x6 possui muito mais células vazias. Mesmo sendo fácil, o número de combinações possíveis já fica muito alto.
...
Esse exemplo tem mais restrições e exigiu mais nós explorados e mais retrocessos do que o 6x6 fácil. Isso mostra que o algoritmo realmente faz tentativas e volta quando encontra caminhos inválidos.
...
Na força bruta, esse é o comportamento principal. O algoritmo gera todas as combinações possíveis e só depois valida o tabuleiro completo. Por isso, no pior caso, a complexidade de busca é exponencial. Além disso, cada combinação precisa ser validada, verificando linhas, colunas e restrições. Mesmo assim, o maior problema continua sendo a quantidade de combinações.
...
O backtracking também tem pior caso exponencial, porque em uma situação muito ruim ele ainda poderia precisar tentar muitas combinações. A diferença é que ele usa poda. Em vez de esperar o tabuleiro todo estar preenchido, ele verifica as regras a cada passo.
